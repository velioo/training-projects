...
async function signUp (ctx, next) {
  ...
  const userId = await Utils.executeInsertUser(connection, userData);
  await Utils.executeInsertTempCode(connection, [userId, tempCode, 'email']);
...
async function confirmAccount (ctx, next) {
  ...
  await Utils.executeUpdateAccountStatus(connection, [userId]);
  await Utils.executeDeleteTempCode(connection, [ctx.params.code]);
