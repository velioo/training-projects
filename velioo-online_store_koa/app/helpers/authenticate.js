module.exports = async (ctx, next) => {
  ...
  const userLoggedInAjaxUrls = [
    '/users/cart/'
  ];
  const employeeLoggedInUrls = [
    '/employee/'
  ];
  ...
  if (userLoggedInUrls.some((url) => requestUrl.startsWith(url) ||
    ...
    if (!ctx.session.isUserLoggedIn) {
      if (userLoggedInAjaxUrls.some((url) => requestUrl.startsWith(url) ||
        (requestUrl + '/').startsWith(url))) {
  ...
  if (employeeLoggedInUrls.some((url) => requestUrl.startsWith(url) ||
    (requestUrl + '/').startsWith(url))) {
    if (!ctx.session.isEmployeeLoggedIn) {
      if (employeeLoggedInAjaxUrls.some((url) => requestUrl.startsWith(url) ||
        (requestUrl + '/').startsWith(url))) {
